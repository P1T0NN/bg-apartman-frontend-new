...
`'offset'` when you need page-number jumps or an accurate total.

Pairing `'offset'` with an `aggregate` builder removes the cap entirely — exact totals and
O(log n) jumps to any page at any table size (§ `aggregate` below).

\* Offset materializes the matched set to count and slice it, so it reads at most
`PAGINATION_DATA.OFFSET_SCAN_LIMIT + 1` rows. Past that cap it reports
`totalCount: null` — the same "unknown" every consumer already understands from cursor
mode: page numbers disappear, prev/next keeps working, nothing throws. Without the cap an
unbounded offset surface walks straight into Convex's hard per-query limits (16,384
documents / 8 MiB) and starts returning 500s with no degradation in between.

Page size is clamped server-side too: `resolvePaginationOpts` bounds `numItems` to
`[1, PAGINATION_DATA.HARD_MAX_PAGE_SIZE]`. Endpoints are a public API — a hand-crafted
request must not be able to ask for a 50,000-row page. Per-request only; every row stays
...
export const fetchProducts = fetchOptimized({
	table: 'products',
	strategy: 'offset',
	aggregate: () => ({ aggregate: aggregateProductsByStatusSortOrder, namespace: 'active' })
});
```
...
- **Filters go through the aggregate too**: exact-match facets via `namespace`, ranges
  ...
  request falls through to `where` (the capped scan) — that is the normal, documented
  arrangement, not a failure.
- **Every write to the table must go through `convex/functions.ts`.** The raw builders
  from `_generated/server` bypass the triggers, and one bypassed write drifts the counter
  permanently. The factory logs a loud `counter drift` error if the aggregate ever hands
  back an id the table doesn't have.
- **Backfill once** after adding an aggregate to a table with existing rows:
  `bunx convex run aggregates:backfill<Name> '{}'`. It is idempotent, so it doubles as the
  repair tool.

### Union + offset is bounded-only BY DESIGN

There is no aggregate escape hatch for union+offset, unlike plain offset. The blocker is
fundamental, not unbuilt: aggregate counters cannot dedupe across specs (a row matching two
specs is counted twice, so the total would be a wrong number rendered with confidence), and
a deduped page jump across merged namespaces has no O(log n) form. If a union surface
outgrows the scan cap, that is the signal to use **cursor mode** (O(perPage · k) forever) or
to denormalize a single access key onto the row so plain offset + aggregate applies.
