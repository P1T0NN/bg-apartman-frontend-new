 ...
 *
 * EVERY read here is index-bounded — no `.filterWith()` post-scans ("THE WALL").
 * The pattern is portable: a project without BA writes the same three-path query
 * (search index | facet-combination index | sort index) against its own `users`
 * table and the page consumes it unchanged.
 */
 ...
 *    `search_email`, O(matches) at any table size. Facets narrow INSIDE the index via
 ...
 *    breaks cursor continuity because the cursor advances in raw result rank).
 *    Results are relevance-ordered; `sortColumn` does not apply while searching.
 *    Pagination is by result rank (`cursor` = rank consumed), capped at
 *    `OFFSET_SCAN_LIMIT` so a crafted cursor cannot force an unbounded read.
 ...
 *    combination — a requested name/email sort falls back to creation time here.
 *
 * 3. **Neither** — `name` / `email_name` / implicit creation index per `sortColumn`,
 *    native `.paginate`.
 *
 * **Frontend needs no changes** for any of this.
 */
export const listUsersPaginated = query({
	...
	handler: async (ctx, args) => {
		...
		if (needle !== null) {
			...
			const rank = args.paginationOpts.cursor
				? Math.max(0, Math.floor(Number(args.paginationOpts.cursor)) || 0)
				: 0;
			if (rank >= PAGINATION_DATA.OFFSET_SCAN_LIMIT) {
				return { page: [], isDone: true, continueCursor: String(rank) };
			}

			const takeN = Math.min(rank + numItems, PAGINATION_DATA.OFFSET_SCAN_LIMIT) + 1;
			...
			const window = raw.slice(rank, rank + numItems);
			const page =
				args.banned === false ? window.filter((r) => (r.banned ?? false) === false) : window;

			return {
				page,
				isDone: raw.length < takeN,
				continueCursor: String(rank + window.length)
			};
		}
