...
/**
 * Permanent **public** URL for an R2 object key, built from `R2_PUBLIC_BASE_URL`
 * (the bucket's public r2.dev / custom-domain base).
 *
 * Prefer this for reads over {@link r2.getUrl}: `getUrl` mints an AWS-presigned
 * URL that expires (default 15 min) and costs a signing round-trip, whereas this
 * is a free, non-expiring string built from the key.
 */
export function r2PublicUrl(key: string): string {
	const base = process.env.R2_PUBLIC_BASE_URL;
	if (!base) throw new Error('R2_PUBLIC_BASE_URL is not set in the Convex environment.');
...
export const { syncMetadata, onSyncMetadata } = r2.clientApi<DataModel>({
	...
	onSyncMetadata: async (ctx, { key }) => {
		...
		// Accept: backfill the permanent PUBLIC URL so reads are free and never
		// expire. (`r2.getUrl` mints a ~15-min presigned URL — costly per read and
		// stale almost immediately.) Fall back to a presigned URL only if the public
		// base isn't configured, so this callback still never throws (a throw here
		// would roll back the cleanup deletes above).
		let url: string;
		try {
			url = r2PublicUrl(key);
		} catch {
