...
export function createSearchInputRemoteHandler<const Sources extends SearchInputRemoteSources>({
	...
	return async (input: SearchInputRemoteSchemaInput): Promise<SearchInputRemoteCommandResult> => {
		...
		if (!source) {
			error(400, 'Unknown search source.');
		}
		...
		if (!rateLimitFallbackKey) {
			error(500, 'Search rate limit key could not be resolved.');
		}
		...
		try {
			...
			const message = cause instanceof Error ? cause.message : 'Search failed. Please try again.';
			error(502, message);
		}
...
async function consumeSearchRateLimit(
	...
	try {
		...
		if (isRateLimitError(cause)) {
			error(429, 'Too many search requests. Please try again shortly.');
		}
...
function resolveTrustedSecret(secretEnvName: string): string {
	...
	if (!secret) {
		error(500, 'Search rate limit is not configured.');
	}
	return secret;
}
...
function defaultSearchRateLimitKey({
	...
	if (!clientAddress) {
		error(400, 'Client address is required for public search.');
	}
