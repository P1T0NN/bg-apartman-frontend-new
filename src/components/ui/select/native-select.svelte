...
{#if useNative}
	...
	<select
		...
		class={cn(
			'select inline-flex min-h-9 w-full cursor-pointer appearance-none items-center rounded-lg border border-input bg-background py-1.5 pr-8 pl-3 text-sm text-foreground outline-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/40 disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-destructive/20',
			className
		)}
...
<style>
	/*
	 * Baseline look lives in Tailwind utilities on the element (so consumer classes can
	 * override via twMerge — component <style> rules are unlayered and would always win).
	 * Only what utilities can't express stays here.
	 */
	.select {
	...
	@supports (appearance: base-select) {
		...
		/* Native tick on the selected option. `order: 1` moves the checkmark (a source-first
		   flex child) to the END so the label stays flush-left; `margin-left: auto` then parks
		   the tick on the right edge. Without `order`, the auto margin shoves the label right. */
