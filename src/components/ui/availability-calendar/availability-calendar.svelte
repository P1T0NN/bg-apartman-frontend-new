<script module lang="ts">
	...
	import type { RangeCalendar as RangeCalendarComponent } from '@/components/ui/range-calendar/index.js';
	...
	export type AvailabilityRange = {
		...
		/** Opaque handle the consumer gets back from `onBookedClick` (the host calendar passes
		 *  the booking id so a sold night can open its reservation). */
		bookingId?: string;
	};
	...
	export type AvailabilityCalendarProps = BaseRangeCalendarProps & {
		...
		/** Makes `booked` cells clickable — they stay unselectable, but a click reports the
		 *  range's `bookingId`. Omit and booked cells are inert as before. */
...
<script lang="ts">
	...
	// COMPONENTS
	import { RangeCalendar, Day as RangeCalendarDay } from '@/components/ui/range-calendar/index.js';
	...
	/** The booking a sold night belongs to, when the consumer wired `onBookedClick`. */
	function bookedIdFor(date: DateValue): string | undefined {
		if (!onBookedClick || isBeforeCalendarMinValue(date)) return undefined;

		return parsedRanges.find((range) => range.status === 'booked' && isDateInRange(date, range))
			?.bookingId;
	}
		...
		>
			{#snippet day({ day: date, outsideMonth }: { day: DateValue; outsideMonth: boolean })}
				{#if !outsideMonth}
					...
					{@const linkedBookingId = booked ? bookedIdFor(date) : undefined}
					<RangeCalendarDay
						data-availability-status={status}
						title={linkedBookingId ? 'Open this reservation' : undefined}
						onclick={linkedBookingId ? () => onBookedClick?.(linkedBookingId) : undefined}
						class={cn(
							...
							// Booked days stay unselectable (still `data-disabled`), but a linked one
							// takes clicks back: the base day class kills pointer events on disabled.
							linkedBookingId &&
								'availability-calendar-day--linked cursor-pointer data-[disabled]:pointer-events-auto',
...
<style>
	...
	/* Affordance for the click-through: only booked cells that actually link somewhere. */
	:global(.availability-calendar-day--linked:hover) {
