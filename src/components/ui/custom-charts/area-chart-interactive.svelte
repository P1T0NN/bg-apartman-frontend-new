<script lang="ts">
	...
	let {
		...
		axis = 'x',
		xAxisFormat,
		// Blank by default: with `axis="x"` there is no left axis to label, and an unformatted
		// value axis prints raw floats. Pass a formatter when opting into `axis={true}`.
		yAxisFormat = () => '',
		...
		/** Hide the built-in range select when the page owns period state (already-windowed data). */
		...
		showLegend?: boolean;
		/** `'x'` = bottom only (default), `true` = both (adds the left value axis), `false` = none. */
		axis?: 'x' | 'y' | boolean;
