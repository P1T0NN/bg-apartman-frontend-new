<script lang="ts" generics="T extends Record<string, unknown>">
	...
	type Props = WithElementRef<HTMLAttributes<HTMLDivElement>> & {
		 ...
		 */
		isSearching?: boolean;
		/** Drop the desktop table's card border & shadow so it sits flush when embedded. */
		borderless?: boolean;
		/** Per-row disclosure detail. Presence adds the chevron gutter + expand affordance. */
		expandedContent?: DataTableExpandedContent<T>;
		/** Accessible name for a row's disclosure button (e.g. its code). */
	...
	// Expansion is view state, not data — it lives here rather than in the page, and resets
	// naturally when a new page of rows replaces these keys.
	let expandedIds = $state<string[]>([]);
	const expandedSet = $derived(new Set(expandedIds));

	function toggleExpand(id: string) {
		expandedIds = expandedSet.has(id) ? expandedIds.filter((x) => x !== id) : [...expandedIds, id];
	}
</script>
...
<div data-slot="data-table" class={cn('flex flex-col gap-0', className)} {...restProps}>
	...
	{:else}
		<Card
			class={cn(
				'hidden gap-0 py-0 md:flex md:flex-col',
			...
			<Table class="min-w-lg">
				...
				<TableHeader>
					<TableRow class="border-b bg-muted/40 hover:bg-muted/40">
						...
						{/if}
						{#if expandedContent}
							<TableHead scope="col" class="h-auto w-10 px-2 py-3">
								<span class="sr-only">Row details</span>
							</TableHead>
						{/if}
