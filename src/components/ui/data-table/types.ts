...
/**
 * Detail rendered under a row when the reader expands it — a real disclosure, not a modal
 * or a navigation. Use it when answering the table's question sometimes needs more than a
 * row can hold (the admin bookings support lookup, AdminPagesSystemDesign.md §3) and a
 * detail route would only duplicate the same fields.
 */
export type DataTableExpandedContent<T> = Snippet<[{ row: T }]>;
