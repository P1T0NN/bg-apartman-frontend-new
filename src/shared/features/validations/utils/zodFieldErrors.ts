...
// per-schema keys) — nothing in this file translates. The Svelte half
...
/**
 * Per-row error CODES for one item of an array field, keyed by the item's own property
 * name (`issues` with path `[arrayKey, index, prop]`). For array editors rendered outside
 * the declared fields — `zodIssuesToFieldErrorCodes` collapses those to the array key
 * alone, which can't say WHICH row failed.
 */
