...
<RangeCalendarPrimitive.Day
	...
	class={cn(
		...
		// Today: primary ring + primary text (no bg fill) so it never reads as part of a
		// selection — selected ends are solid primary, range middle is the accent fill.
		'data-[range-middle]:rounded-none [&[data-today]:not([data-selected])]:font-semibold [&[data-today]:not([data-selected])]:text-primary [&[data-today]:not([data-selected])]:ring-1 [&[data-today]:not([data-selected])]:ring-primary/60 [&[data-today]:not([data-selected])]:ring-inset [&[data-today][data-disabled]]:text-muted-foreground',
		// range Start
