...
/**
 * Subset of `useQuery`'s return that the list components actually read, plus `refetch`.
 *
 * `refetch` is the one thing a subscription gives you for free and a one-shot read does not:
 * a way to see your own writes. Without it, a list that mutates its own rows (a bulk delete,
 * a row action) would sit on stale data until its args changed — which is exactly the reason
 * such screens would otherwise be forced to subscribe. `useQuery` has no equivalent because
 * it never needs one, so the wrappers supply a no-op for the realtime branch.
 */
export type OneShotQueryReturn<T> = {
	readonly data: T | undefined;
	readonly error: Error | undefined;
	readonly isLoading: boolean;
	/** Re-run the query with the current args. Sequence-guarded like any other run. */
	refetch: () => void;
};
...
export function convexOneShotQuery<Query extends FunctionReference<'query'>>(
	...
	return {
		...
		// Reads `getArgs()` fresh so a refetch after a mutation can't replay stale args.
		refetch: () => run(getArgs())
	};
}
